...
class recorder:
	...
	def tf_callback(self,tf_msg):
		...
		if(tf_msg.transforms[0].child_frame_id=='base_link'):
	...
	def move(self):
		...
		X_map=[-100+0.05*(ind%4000) for ind,val in enumerate (map_arr) if val>0] #x cordinates of wall points and cylinder obstacle points
		Y_map=[-100+0.05*(ind//4000) for ind,val in enumerate (map_arr) if val>0] #y cordinates of wall points and cylinder obstacle points
		#plt.scatter(X_map,Y_map) # uncomment to plot the points
		#plt.show() # uncomment to plot the points
		
		while not rospy.is_shutdown():
			if not scan_arr:
				scan_arr = list(np.ones(360))	# GAZEBO: 360 / REAL ROBOT: 1153
			else:
				scan_arr = recorder.scan_data
			...
			#Reading LIDAR scan points FOR REAL ROBOT
			X_scan=[-val*np.cos(ind*3.14/180*resL) for ind,val in enumerate (scan_arr) if val<2.5]
			Y_scan=[-val*np.sin(ind*3.14/180*resL) for ind,val in enumerate (scan_arr) if val<2.5]
			...
			if flag:
				...
				#rx = np.multiply(rx,0.1)
				#ry = np.multiply(ry,0.1)
				ry = np.subtract(ry,100)
				print("path",rx,ry)
			v_cmd,w_cmd,ed,edi= stage1(est_pos,rx,ry) # You can change the inputs or function as you wish
				#v_cmd = 0
				#w_cmd = 0
			#print(v_cmd,w_cmd)
			#print(rx, ry)
